...
def display_lines(image, lines):
    ...
    if lines is not None:
        for line in lines:
            # print(line)    //prints the array containing line with 2D array
...
while(cap.isOpened()):
    ...
    lines = cv2.HoughLinesP(cropped_image, 2, np.pi / 180, 100, np.array([]), minLineLength=40,
                            maxLineGap=5)  # (image, precision of 2 pixel, with 1 degree precision i.e. in radians, threshold- no. of intersection for precision, placeholder array)
    # cv2.HoughLinesP(rho(precision in hough space), theta(angle between points of intersection), threshold(no. of intersections), placeholder array, length of a line in pixel that we will accept in input)
